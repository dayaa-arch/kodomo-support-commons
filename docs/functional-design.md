...
- 共有する参照データ（区・隣接区、緊急相談先、外部リンク）と UI プリミティブは `shared` に置く。

**緊急相談先の情報源**: 施設検索の対象ではなくサイト全体で固定して案内する情報のため、区マスタと同様に `shared/domain/emergency-contacts.ts` の参照データとして持つ（子ども向け相談窓口2件＋通報先110/119）。正本データセットの `emergency_resources` にも近い情報があるが、チャイルドラインが未収録であり粒度も異なるため、**UI は `shared` 側を情報源とする**。両者の統一は Step 4 以降の課題とする。
...
- カード表示項目: 表示順 / 施設名 / 運営主体 / 説明 / 区名（「選択した区」「市全域から利用可」「隣接区」を区別）/ 相談方法 / 特徴ラベル（費用・相談条件・確認状況）/ 詳細への導線。
- **同じ情報を1枚のカードに二度置かない**。費用は特徴ラベルにのみ出す。
- 説明は `summary` を優先し、未登録なら出典で確認できている対象年齢（`whoCanUse`）を代わりに表示する。どちらも無い場合は行自体を出さない（「情報がありません」とは書かない）。
...
│ ═══ 今すぐ助けが必要な場合 ═══      │
│ (110番・119番／公的相談窓口・固定表示)│
└──────────────────────────────┘
```

**タグと表の役割分担**（同じ画面で同じ情報を二度見せない）:

| 面 | 内容 |
| --- | --- |
| 特徴ラベル | 出典で**確認できた**特徴（費用・予約・匿名相談・保護者のみ相談・確認状況） |
| 表 | 特徴ラベルに出ない詳細（相談内容・対象・流れ・相談方法・電話・所在地・条件・受付時間・補足・出典・最終確認日） |
| 未掲載まとめ | 出典で確認できなかった**項目名の列挙**（末尾に1箇所） |

- 確認できていない事実は特徴ラベルに出さない（「不明」を特徴として並べない）。
- 値が無い項目は表から省き、未掲載まとめに項目名を回す。空欄のまま放置はしない。
