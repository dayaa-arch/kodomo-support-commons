...
- **ドメインロジックは単体テスト必須**: 一致度スコアリング、区・隣接区の抽出、確認状況・特徴ラベル導出、未掲載項目の表示置換。
- **データ検証テスト**: スキーマ検証が必須項目欠落・enum 不一致を確実に弾くこと。
...
- **施設情報の更新**: `data/facilities/*.yaml` の追加・修正は PR で行う。公開前に運営者または承認メンテナーのレビューを**必須**とする。
